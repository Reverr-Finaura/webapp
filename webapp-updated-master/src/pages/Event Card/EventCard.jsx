    ...
    <>
      {meetingArray.map((item, index) => {
        return (
          ...
          >
            <img
              className='meeting-person-photo'
              src={
                purementorArray.filter((items) => {
                  ...
                  : "./images/noMentorImage.png"
              }
              alt='photo'
            />
            <div className='meeting-person-name-date-cont'>
              <h4 className='meeting-person-name'>{`Connect with ${
                purementorArray.filter((items) => {
                  return items.email === item.mentor_email;
                })[0].name
              }`}</h4>
              <p className='meeting-person-date'>
                {toMonthName(item.month) + " " + item.date}
              </p>
            </div>

            {item.approved ? (
              <button className='send-invite-btn'>Check Email</button>
            ) : (
