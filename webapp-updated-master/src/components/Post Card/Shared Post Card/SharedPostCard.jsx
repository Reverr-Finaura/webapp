...
const SharedPostCard = ({
  ...
  setPostsData,
  postsData,
  setEditSinglePostIsClick,
  handleEditSinglePost,
}) => {
  ...
  const [singlePostCommentIconClick, setsinglePostCommentIconClick] =
    useState(false);
  ...
  const [PostThreeDotsClickCommentId, setPostThreeDotsClickCommentId] =
    useState(null);
    ...
    <>
      <section id={styles.SharedPostCard}>
        <div
          ...
          className={styles.SharedPostCardTop}
        >
          <img
            style={{
              width: "40px",
          ...
          <div className='postUserDetail'>
            ...
            <p className={styles.postAuthorDesignation}>
              {postUserDetail?.designation ? postUserDetail?.designation : ""}
            </p>
          </div>
            ...
            <h3 className={styles.postText}>
              {ShowMorePostText ? (
                ...
                <>
                  ...
                  <span
                    onClick={() => setShowMorePostText(true)}
                    className={styles.morePostTextButton}
                  >
                    ...continue
                  </span>{" "}
                </>
        ...
        <div className={styles.postLikesAndCommentContainer}>
          <div className={styles.postLikesContainer}>
            <div
              ...
              className={styles.postLikesContainerLikeIcon}
            >
              {item?.likes.includes(user?.user?.email) ? (
                <AiTwotoneLike className='postLikesContainerLikedIconn' />
              ) : (
                <AiOutlineLike className='postLikesContainerLikeIconn' />
              )}
            </div>
            <p className={styles.postSharedLikeCountText}>
          ...
          <div className={styles.postCommentContainer}>
            <div className={styles.commentContainer}>
              <FaComments
                onClick={() =>
                  setsinglePostCommentIconClick((current) => !current)
                }
                className={styles.commentPostIconn}
              />
          ...
          <div
            ...
            className={styles.postSendLinkContainer}
          >
          ...
          <div className={styles.threeDotsMainCont}>
            <div className={styles.threeDotsContainer}>
              ...
              <div style={{ display: "flex" }}>
                <TfiMoreAlt
                  onClick={() => setThreeDotsClicked((current) => !current)}
                ...
                <div
                  className={
                    user?.user?.email === item?.postedby?.id
                      ? styles.threeDotsOptions
                      : styles.standardThreeDotsOption
                  }
                >
                  {user?.user?.email === item?.postedby?.id ? (
                    <div
                      onClick={() => handleDeletePostButtonClick(item.id)}
                      className={styles.threeDotsDeletePostOption}
                    >
                      Delete Post
                    </div>
                  ) : null}
                  {user?.user?.email === item?.postedby?.id ? (
                    ...
                    >
                      <div
                        onClick={() => {
                          handleEditSinglePost(item, item.id);
                          setThreeDotsClicked(false);
                          setEditSinglePostIsClick(true);
                        }}
                        className={styles.threeDotsEditPostOption}
                      >
                        Edit Post
                      </div>
                    </a>
      ...
      <section className={styles.newCommentOnPostSection}>
        {editCommentButtonIsClick ? (
          ...
          <section
            style={{ display: singlePostCommentIconClick ? "" : "none" }}
            className={styles.uploadPostContainerrrrSection}
          >
            <div className={styles.newCommentContainerrrr}>
              <img
                className={styles.communityNewCommentContUserImage}
                src={
                  userDoc?.image
                    ? userDoc.image
                    : "https://media.giphy.com/media/KG4PMQ0jyimywxNt8i/giphy.gif"
              ...
              <div className={styles.textAreaUploadContainer}>
                <textarea
                  className={item?.id}
                  onChange={(e) => setSinglePostNewComment(e.target.value)}
                  name='newComment'
                  id={styles.postCommentContainerExpanded}
                  rows='3'
                  placeholder='Share Your Thoughts'
                  value={singlePostNewComment}
                ></textarea>

                <div className={styles.addImageandUploadPostIcon}>
                  <button
                    onClick={() => handleNewCommentonPost(item, item.id)}
                    className={styles.uploadPostIconButton}
                  >
                    Comment
                  </button>
                </div>
                  ...
                  <div className='commentedByAndComment' key={list.commentid}>
                    <div className='commented-by-and-edit-cont'>
                      <img
                        ...
                        src={
                          commentedByUserDetail?.filter((it) => {
                      ...
                      {list?.commentedby?.id === user?.user?.email ? (
                        <TfiMoreAlt
                          ...
                          onClick={() => {
                            setCommentThreeDotsClicked((current) => !current);
                            setPostThreeDotsClickCommentId(list?.commentid);
                          }}
                        />
                      ) : null}
                      ...
                      {CommentThreeDotsClicked &&
                      list?.commentedby?.id === user?.user?.email &&
                      PostThreeDotsClickCommentId === list?.commentid ? (
                        <>
                          <div className='threeDotsOptions commentThreeDotsOption'>
                            ...
                            >
                              <div
                                onClick={() => {
                                  setCommentThreeDotsClicked(false);
